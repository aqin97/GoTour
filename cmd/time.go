...
var calcTimeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			log.Fatalf("timer.GerCalculateTime err: %v", err)
		}
...
func init() {
	...
	calcTimeCmd.Flags().StringVarP(&calculateTime, "calculate", "c", "", `计算需要的时间，时间戳或已格式化后的时间有效`)
